...
describe('Technical Analysis Library', () => {
  describe('Moving Averages', () => {
    it('calculateSMA() returns correct simple moving average', () => {
      ...
      assert.ok(sma5 !== null);
      assert.equal(typeof sma5, 'number');
      // SMA of last 5 prices: (15+16+17+18+19)/5 = 17
      assert.equal(sma5, 17);
    });
    ...
    it('calculateSMA() handles invalid inputs', () => {
      ...
      assert.equal(calculateSMA([10, 11], 5), null); // Insufficient data
    });
    ...
    it('detectMACrossover() handles no crossover', () => {
      ...
      assert.ok(result !== null);
      assert.equal(result.crossover, false);
  ...
  describe('Momentum Indicators', () => {
    ...
    it('calculateMACD() returns histogram with correct structure', () => {
      const prices = Array.from({ length: 35 }, (_, i) => 100 + Math.sin(i / 5) * 10);
      const macd = calculateMACD(prices, 12, 26, 9);

      assert.ok(macd !== null);
      assert.ok(typeof macd === 'object');
      assert.ok('macd' in macd);
  ...
  describe('Trend Indicators', () => {
    it('calculateADX() returns trend strength', () => {
      ...
      assert.ok(adx !== null);
      assert.ok(typeof adx === 'object');
      assert.ok('adx' in adx);
    ...
    it('determineTrend() identifies neutral/ranging market', () => {
      ...
      // With sideways action, should likely be NEUTRAL but allow for variations
      assert.ok(['NEUTRAL', 'BULLISH', 'BEARISH'].includes(trend.direction));
    });
  ...
  describe('Support/Resistance', () => {
    ...
    it('checkSupportResistance() detects proximity to level', () => {
      const price = 1.1025;
      const levels = [
        { value: 1.1, type: 'SUPPORT' },
        { value: 1.1025, type: 'RESISTANCE' },
        { value: 1.105, type: 'RESISTANCE' },
      ];
